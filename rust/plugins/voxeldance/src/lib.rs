...
pub struct VoxelDancePlugin { meta: PluginMeta, capabilities: SoftwareCapabilities }

impl Default for VoxelDancePlugin { fn default() -> Self { Self::new() } }

impl VoxelDancePlugin {
    pub fn new() -> Self {
        Self {
            meta: PluginMeta {
                id: "com.aidesign.voxeldance".into(), name: "VoxelDance Additive".into(),
                version: env!("CARGO_PKG_VERSION").into(), script_language: "python".into(),
            },
            capabilities: SoftwareCapabilities {
                actions: vec!["模型导入".into(),"支撑生成".into(),"切片".into(),"路径规划".into(),"导出GCode".into()],
                file_formats: vec!["vda".into(),"stl".into(),"gcode".into(),"3mf".into()],
                constraints: None,
            },
        }
    }
...
impl DesignPlugin for VoxelDancePlugin {
    ...
    fn execute(&self, script: &str) -> Result<ScriptResult, String> {
        Ok(ScriptResult::success(Some(format!("[VoxelDance Additive] 脚本已生成:\n\n{}", script)), vec![]))
    }
    fn preview(&self, script: &str) -> Result<ScriptResult, String> {
        Ok(ScriptResult::success(Some(format!("[预览] VoxelDance 脚本:\n{}", script)), vec![]))
    }
