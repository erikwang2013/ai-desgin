...
pub struct WaxJetPrintPlugin { meta: PluginMeta, capabilities: SoftwareCapabilities }

impl Default for WaxJetPrintPlugin { fn default() -> Self { Self::new() } }

impl WaxJetPrintPlugin {
    pub fn new() -> Self {
        Self {
            meta: PluginMeta {
                id: "com.aidesign.waxjetprint".into(), name: "WaxJetPrint".into(),
                version: env!("CARGO_PKG_VERSION").into(), script_language: "python".into(),
            },
            capabilities: SoftwareCapabilities {
                actions: vec!["蜡模导入".into(),"模型优化".into(),"支撑生成".into(),"切片".into(),"导出GCode".into()],
                file_formats: vec!["stl".into(),"wax".into(),"gcode".into()],
                constraints: None,
            },
        }
    }
...
impl DesignPlugin for WaxJetPrintPlugin {
    ...
    fn execute(&self, script: &str) -> Result<ScriptResult, String> {
        Ok(ScriptResult::success(Some(format!("[WaxJetPrint] 脚本已生成:\n\n{}", script)), vec![]))
    }
    fn preview(&self, script: &str) -> Result<ScriptResult, String> {
        Ok(ScriptResult::success(Some(format!("[预览] WaxJetPrint 脚本:\n{}", script)), vec![]))
    }
