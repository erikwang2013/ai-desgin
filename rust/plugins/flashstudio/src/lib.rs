...
pub struct FlashStudioPlugin { meta: PluginMeta, capabilities: SoftwareCapabilities }

impl Default for FlashStudioPlugin { fn default() -> Self { Self::new() } }

impl FlashStudioPlugin {
    pub fn new() -> Self {
        Self {
            meta: PluginMeta {
                id: "com.aidesign.flashstudio".into(), name: "Flash Studio".into(),
                version: env!("CARGO_PKG_VERSION").into(), script_language: "python".into(),
            },
            capabilities: SoftwareCapabilities {
                actions: vec!["模型编辑".into(),"支撑生成".into(),"切片".into(),"打印管理".into(),"导出GCode".into()],
                file_formats: vec!["stl".into(),"obj".into(),"gcode".into()],
                constraints: None,
            },
        }
    }
...
impl DesignPlugin for FlashStudioPlugin {
    ...
    fn execute(&self, script: &str) -> Result<ScriptResult, String> {
        Ok(ScriptResult::success(Some(format!("[Flash Studio] 脚本已生成:\n\n{}", script)), vec![]))
    }
    fn preview(&self, script: &str) -> Result<ScriptResult, String> {
        Ok(ScriptResult::success(Some(format!("[预览] Flash Studio 脚本:\n{}", script)), vec![]))
    }
