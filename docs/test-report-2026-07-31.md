...
|----|------|---------|--------|------|------|------|
| Dart/Flutter | flutter_test | 12 | 49 | **49** | 0 | ~11s |
| Rust | cargo test | 18 crates | **0** | 0 | 0 | ~6s |
...
**Dart 端**: 49 个测试覆盖核心逻辑（任务编排、模型路由、插件管理、会话持久化）和基础 UI 渲染。测试质量中等 — 核心类的 CRUD 和路由匹配覆盖充分，但缺少并发、异常恢复和 UI 交互流程的深度测试。

**Rust 端**: 0 个测试。18 个 crate 均可正常编译但完全没有测试用例。`execute()` 中包含了真实外部进程调用和网络请求，这些风险最高的代码路径反而是最缺测试的。

**整体评估**: Dart 端 49/49 通过，lint 0 警告；Rust 端编译通过但 0 测试覆盖。后续应优先为 Rust core types 和关键 plugin 添加单元测试。
